...
mod surface_region {
    ...
    static REGIONS: OnceLock<Mutex<HashMap<String, RegionState>>> = OnceLock::new();

    fn regions() -> &'static Mutex<HashMap<String, RegionState>> {
    ...
    fn apply(
        ...
        // Acrylic 绘制在整个原生矩形窗口上，CSS 圆角无法裁剪它；窗口区域须同步裁剪。
        let diameter = (radius.clamp(0.0, 100.0) * scale * 2.0).round() as i32;
        let region = unsafe {
            CreateRoundRectRgn(
                0,
                0,
        ...
        // 成功后 GDI 区域由系统持有；尺寸和圆角未变时避免重新分配。
        regions()
            .lock()
            .map_err(|error| error.to_string())?
            .insert(label.to_string(), current);
        Ok(())
    }

    pub fn set(window: &WebviewWindow, enabled: bool, radius: f64) -> Result<(), String> {
        if window.label() == "main" {
            return Ok(());
        }
        if !enabled {
            let had_region = regions()
                .lock()
                .map_err(|error| error.to_string())?
                .remove(window.label())
                .is_some();
            if had_region {
                ...
                if unsafe { SetWindowRgn(hwnd, std::ptr::null_mut(), 1) } == 0 {
                    return Err("SetWindowRgn reset failed".into());
                }
            }
            return Ok(());
        }
        ...
        apply(window.label(), hwnd, size, scale, radius)
    }

    pub fn resized(window: &Window) {
        let radius = regions()
            .lock()
            .ok()
            .and_then(|regions| regions.get(window.label()).map(|state| state.radius));
        if let Some(radius) = radius {
            ...
            if let Err(error) = result {
                eprintln!("failed to resize surface region: {error}");
            }
        }
    }
...
pub fn set_surface_region(
    ...
    surface_region::set(window, enabled, radius)
}
...
pub fn handle_window_event(window: &Window, event: &WindowEvent) {
    ...
    if matches!(event, WindowEvent::Resized(_)) {
        surface_region::resized(window);
    }
    if matches!(event, WindowEvent::Destroyed) {
        #[cfg(target_os = "windows")]
        surface_region::remove(window.label());
