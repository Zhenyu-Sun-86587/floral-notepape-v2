...
- P6 Windows 修复（1.3.1）：用户发现开机静默启动后，用单便签快捷键关闭唯一的便签会让应用退出，快捷键无法再次恢复。Tauri 在最后一个窗口销毁时发送无退出码的 `ExitRequested`；现在 Windows 对这种隐式退出保持托盘进程和全局快捷键，托盘“退出”等显式退出仍生效。用户 GUI 复测待完成。
