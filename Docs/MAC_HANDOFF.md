...
## 1.7.7 悬停预览生命周期（Mac 未实测）

新增活动预览的原生光标位置检查，关闭使旧会话失效，隐藏时卸载前端内容。Windows 额外读取左键状态兜底窗口外释放；Mac 仍使用 pointerup/cancel、blur 及返回窗口后的 buttons 状态释放文本选择占用，需要重点验收窗外松键和跨应用切换。参见 [RELEASE_1.7.7](RELEASE_1.7.7.md)。
...
- 1.7.0 悬停初值约 250ms；离开胶囊和预览整体后约 550ms 收起，owner/会话判定优先于延时。轨道不随悬停变宽或移动，不显示浏览器默认 tooltip。
- 共享预览按需读取正文，最多 60 行/2400 字符并标记截断；复用安全 Markdown 渲染，较重的 Mermaid/公式由现有管线按需处理。显示数据后再呈现窗口，generation 过期请求不得覆盖新预览。
