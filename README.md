...
把 Markdown 笔记固定在桌面，需要时直接写，暂时不用时收纳到屏幕边缘。当前开发版本为 **1.7.6**。
...
- **外观与唤起**：主题、字体、背景与透明度可调整；支持托盘、静默启动和应用/单便签快捷键。Windows Acrylic 由系统控制，桌面附着模式不使用 Acrylic。
  ...
  1.7.6 移除胶囊悬停预览的正文截断，长文可在预览内滚动查看全文；胶囊拖动头与成员按实际窗口比例分配空间，修正末端裁切。保留 1.7.5 Editor 架构。范围与待验收项见 [发布说明](Docs/RELEASE_1.7.6.md)。

## 下载安装

1.7.6 本地构建安装包位于 `src-tauri/target/release/bundle/nsis/`；本轮只推送源码，不创建 GitHub Release。下方保留上一版已发布下载。
